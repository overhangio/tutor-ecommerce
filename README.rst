...
Cybersource config values

What `transaction_key` means here is SOAP API key and can be generated by going to Key Management under Payment Configuration and then selecting generate key and then soap api toolkit.

`flex_shared_secret_key` is the key generated from <https://developer.cybersource.com/api/developer-guides/dita-gettingstarted/authentication/createSharedKey.html> and 
`flex_shared_secret_key_id` is the shared secret key.

You should be able to get `merchant_id` easily upon loggin in to your cybersource account.
